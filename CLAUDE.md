...
| `017_driver_career_earnings.sql` | driver-side lifetime earnings (Phase 10): `driver_earning_kinds` data table (`race_salary` / `carry_in` / `adjustment`) and `driver_earnings_ledger`, keyed on `(guild_id, member_id)` so a career total carries across seasons with no carry step. FKs are `ON DELETE SET NULL`, not `CASCADE` — deleting an old season must not erase career history. Partial unique `uq_driver_earnings_race_salary (contract_id, round_id) WHERE kind = 'race_salary'` makes a re-imported round harmless. Touches no budget table: earnings are a record of what a driver was paid, not a second transfer |
