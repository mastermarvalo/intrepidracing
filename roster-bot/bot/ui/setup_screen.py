...
class _SeasonButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_modal(_SeasonModal(view))
...
class _TierButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        assert isinstance(view, SetupView)
        # Suggest the next free slot so consecutive adds don't collide on
        # rank order and silently sort unpredictably.
        suggested = len(view.status.tiers) + 1
        await interaction.response.send_modal(
            _TierModal(view, suggested_rank=suggested)
        )
